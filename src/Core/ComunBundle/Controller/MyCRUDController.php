        ...
        try {
            ...
            if ($id != null) {
                ...
                if (count($id) > 0) {
                    ...
                    $result["success"] = true; // pass custom message(useful for getting status of group actions)
                    if (count($id) == 1)
                        $result["sMessage"] = "Item deleted.";
                    else
                        $result["sMessage"] = "Items deleted.";
                }
            }
...
//            ldd($e);
            $data = $this->get($this->tableModelService)->getTransformedData();
            return new JsonResponse(array_merge_recursive(array('success'=>false,'sMessage'=>'You cannot delete a referenced item.','code'=>$e->getMessage()),$data));
        }
    }
    /**
     * @return Response
    ...
    public function editAction()
    {
        ...
        if($id) {
            ...
            return $this->creadeUpdate($obj, $this->editView, "Item modified.");
        }
        else
            return new JsonResponse(array("success" => false, "sStatus" => "ERROR", "sMessage" => "Inexistent item."));
    }
    ...
    protected function creadeUpdate($obj,$view,$msg =  "Item created.",$extra_params=array())
    {
        $form = null;
        $redirect = true;
...
//                ldd($form->getErrorsAsString());
            ...
            elseif($redirect == false){
                return new JsonResponse(array("success" => false, "reload" => false, "sMessage" => "Remain Fields with non valid values."));
            }
            else {
    ...
    protected function extraData($obj)
    {
        die(2);
        return $obj;
    }
